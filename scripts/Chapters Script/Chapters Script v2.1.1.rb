...
# Paramters:
#-------------------------------------------------------------------------------
CH_PATH = "Graphics/Pictures/Chapters/Chapter"
...
class Scene_Chapter < SMScreen
  def initialize(chapter, introTrans, endTrans, xPos, yPos)
    super(CH_PATH,chapter.to_s,xPos,yPos,$topScreen.getViewport)
    @chapter=chapter
  ...
  def pbStartScene
    ...
    addObj("blScreen",@x,@y,"BlScreen")
    @objectsList["blScreen"].opacity=0
    @sprite.z=9999
  ...
  def pbUpdate
    ...
    if @introTrans
      FADE_TO_BLACK.times do
        pbWait(1)
        Graphics.update
        @objectsList["blScreen"].opacity=@objectsList["blScreen"].opacity+255/FADE_TO_BLACK
      end
    else
      @objectsList["blScreen"].opacity=255
    end
    ...
    FADE_IN_TEXT.times do
      pbWait(1)
      Graphics.update
      @sprite.opacity=opacity+255/FADE_IN_TEXT
    end
    ...
    FADE_OUT_TEXT.times do
      pbWait(1)
      Graphics.update
      @sprite.opacity=opacity-255/FADE_OUT_TEXT
    end
    ...
    if @endTrans
      FADE_OUT_BLACK.times do
        pbWait(1)
        Graphics.update
        @objectsList["blScreen"].opacity=@objectsList["blScreen"].opacity-255/FADE_OUT_BLACK
      end
    end
  end
