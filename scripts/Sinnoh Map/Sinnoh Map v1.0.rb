...
#Cursor handling ---------------------------------------------------------------
  ...
  def update(locId)
    ...
    if !DOUBLESCREEN and (Input.triggerex?(0x4B) or        #0x4B == 'K' Key
  ...
  def bookTape(mode)
    self.opacity=255 if mode==1
    16.times do
      self.zoom_y=zoom_y()+(mode*24).to_f/255
      pbWait(1)
    end
...
class MapBtmScreen < SMScreen
  def initialize(name,x,y,viewport,managerRef)
    ...
    lText=[[_INTL("GUIDE MAP"),x+256,y,2,Color.new(87,87,87),Color.new(180,180,180)]]
    ...
    descTitle = SMSprite.new(@sprite.x,@sprite.y,MAP_PATH+"SinnohMap",@spriteViewport,self)
    descTitle.clear
    descTitle.setSystemFont
    @objectsList["descPanel"].insertObj(descTitle,"descTitle")
    
    descText = SMSprite.new(@sprite.x,@sprite.y,MAP_PATH+"SinnohMap",@spriteViewport,self)
    descText.clear
  ...
  def curtain(mode=1)
    16.times do
      zy = @objectsList["blCurtain"].zoom_y
      @objectsList["blCurtain"].zoom_y=zy+24*mode
      pbWait(1)
    end    
  end
  ...
  def showDesc(locId)
    if locId != -1
      ...
      if !path.include?("Route") || SHOW_DESC_ON_ROUTES
        ...
        @objectsList["descPanel"].objectsList("minimap").bitmap=path
  ...
  def update(locId)
    if !@detailMap
      if DOUBLESCREEN
        if @objectsList["Button"].leftClick?
          pbBtnAnimation(@objectsList["Button"].sprite,5,EXIT_MAP_SE)
