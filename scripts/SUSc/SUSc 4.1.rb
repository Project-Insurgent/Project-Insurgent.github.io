#==============================================================================#
#                       SOMERSAULT'S UTILITIES SCRIPT v4.1                     #
#                              Give credit if used                             #
#==============================================================================#
...
#(aka, when you press escape in the selectName scene)
#==============================================================================#
...
class SMSprite < SMObject
 ...
 #GETTERS:
  ...
  #meant for zoom_x, zoom_y, and angle  
  def saveAndLoadOX(key,newOx=@sprite.ox)
    ret = @objectsList[key].ox
    @objectsList[key].ox=@sprite.x-@objectsList[key].x+newOx
    return ret
  end
  
  def saveAndLoadOY(key,newOy=@sprite.oy)
    ret = @objectsList[key].oy
    @objectsList[key].oy=@sprite.y-@objectsList[key].y+newOy
    return ret
  end
  
  def saveOrigins(key,newOx,newOy)
    ox = saveAndLoadOX(key,newOx)
    oy = saveAndLoadOY(key,newOy)
    return [ox,oy]
  end
  
  def zoom_x=(val); set_zoom_x(val); end
  def zoom_y=(val); set_zoom_y(val); end
  
  def set_zoom_x(val,_ox=@sprite.ox)
    @sprite.zoom_x=val
    for i in @objectsList.keys
      origin = saveAndLoadOX(i,_ox)
      @objectsList[i].set_zoom_x(val,@objectsList[i].ox)
      @objectsList[i].ox = origin
    end
  end
  
  def set_zoom_y(val,_oy=@sprite.oy)
    @sprite.zoom_y=val
    for i in @objectsList.keys
      origin = saveAndLoadOY(i,_oy)
      @objectsList[i].set_zoom_y(val,@objectsList[i].oy)
      @objectsList[i].oy = origin
    end
  end
  
  def angle=(val,_ox=@sprite.ox,_oy=@sprite.oy)
    for i in @objectsList.keys
      origins = saveOrigins(i,_ox,_oy)
      @objectsList[i].angle(val,@objectsList[i].ox,@objectsList[i].oy)
      restoreOrigins(i,origins)
    end
    @sprite.angle=val
  end
  ...
  def bitmap=(val); @sprite.bitmap=Bitmap.new(val); end
