...
class SinnohMapTopScreen < SMSpriteWrapper
  ...
  def update
    ...
    if !MULTI_SCREEN && !@detailMode && Input.trigger?(Input::USE)
      curtainEffect(10,-1,"v")
      self.visible    = false
...
class SinnohMapView < SMModuleView
  ...
  def initialize(mod,ctrl=nil)
    ...
    #init bottom screen:
    insertPanel(:SECN_PANEL,SinnohMapBtmScreen.new(@mod,$scrManager.list[:SECN_PANEL].viewport,self))
    
    #init top screen:
    insertPanel(:MAIN_PANEL,SinnohMapTopScreen.new(@mod,$scrManager.list[:MAIN_PANEL].viewport,self))
    #@panels[:SECN_PANEL].fitSpriteInViewport(@panels[:SECN_PANEL].viewport)
    #@panels[:MAIN_PANEL].fitSpriteInViewport(@panels[:MAIN_PANEL].viewport)

    #pbMessage(_INTL("{1}",@panels[:SECN_PANEL].list["detMap"]))
    @panels[:SECN_PANEL].list["detMap"].list["det_Bg"].zoom_x*=2
    @panels[:SECN_PANEL].list["detMap"].list["det_Bg"].zoom_y*=2
    fitInViewports
    deactivate
  end
  ...
  def execute
    ...
    @panels[:SECN_PANEL].detailMode ||= (MULTI_SCREEN && @panels[:SECN_PANEL].list["Button"].leftClick?(5,0)) || Input.trigger?(Input::USE)
    #@panels[:SECN_PANEL].detailMode &&= !Input.trigger?(Input::BACK) 
    @panels[:SECN_PANEL].update
    
    #refresh top screen:
    @panels[:MAIN_PANEL].visible = MULTI_SCREEN || !@panels[:SECN_PANEL].detailMode #? @panels[:MAIN_PANEL].activate : @panels[:MAIN_PANEL].visible = false
