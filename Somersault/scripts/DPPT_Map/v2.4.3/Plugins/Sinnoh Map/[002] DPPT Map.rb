...
class SinnohMapView < SMModuleView
  ...
  def initialize(mod,ctrl=nil)
    ...
    @mpZ = $scrManager.list[:MAIN_PANEL].viewport.z
    $scrManager.list[:MAIN_PANEL].viewport.z = $scrManager.list[:SECN_PANEL].viewport.z+1
  end
  ...
  def finish
    super()
    $scrManager.list[:MAIN_PANEL].viewport.z = @mpZ
  end
