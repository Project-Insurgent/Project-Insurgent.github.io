...
=end
        ...
        while !(line.include? "[") && !file.eof?
          if !(line.include? SEPARATOR)
            ...
            locN = line[0] #name
            puts(_INTL("Inserted location name: "+locN)) if $DEBUG
            
            locI = line[1] if line.length > 1 #icon
            puts(_INTL("Inserted location icon: "+locI)) if locI.to_i != -1 and $DEBUG
            line = file.readline()
            
            locCS = line.split(',',-1) #position and size
            puts("Inserting location coord and size:")
            for i in 0...locCS.length-1; puts(_INTL("Inserted value: " + locCS[i])); end  if $DEBUG

            line = file.readline()
            locM = line.split(',',-1) #map(s) IDs
            puts(_INTL("Inserting location map IDs: "))
            for i in 0...locM.length-1; puts(_INTL("Inserted location map ID: " + locM[i])); end  if $DEBUG
            ...
            puts(_INTL("Inserted location description: "+ret2[locN] + "\n"))  if $DEBUG
            ...
            for i in 0...locM.length-1
              ...
              ret1[locM[i].to_i][5] = locI.to_i if i == 0 && locI.to_i != -1
            end
          end
