#==============================================================================#
#                       SOMERSAULT'S UTILITIES SCRIPT v4.0                     #
#                              Give credit if used                             #
#==============================================================================#
...
#==============================================================================#
SM_SCREEN_SYSTEM_WIDTH  = 512
SM_SCREEN_SYSTEM_HEIGHT = 384
...
# (Press ctrl + shift + F to access the search tab). Done!
 TEXTLENGTH = 10
 USEKEYBOARD = false  # To change between a keyboard input or a char table input 
#==============================================================================#
#==============================================================================#
#                              SET CHARACTER NAME                              #
#==============================================================================#
...
#                                  DIGIT FORMAT                                #
#==============================================================================#
# An easy function to change the number of digits (and zeroes at the left) that
...
def pbNext_mrua_Pos(sprite,speed=[0,0],deltaTime=1,g=1)
  sprite.x = (sprite.x + speed[0]*deltaTime).to_i
  sprite.y = (sprite.y - speed[1]*deltaTime + (g*deltaTime*deltaTime)/2).to_i
  sy = speed[1] - g*deltaTime
  _speed = [speed[0],sy]
  return _speed
end
...
=begin
alias pbMEPlayOld pbMEPlay
def pbMEPlay(param,volume=nil,pitch=nil)
  aux = param
  pbMEPlayOld(param + "_Intro",volume,pitch) if FileTest.audio_exist?("Audio/ME/"+ param+"_Intro") && param != $introName
  $introName = aux
  pbMEPlayOld(param,volume,pitch)
end
=end
#==============================================================================#
#==============================================================================#
...
#==============================================================================#
def pbApplyRescaleRel(pic,relOX,relOY,scale)
  if pic
    pic.zoom_x = pic.zoom_x * scale
    pic.zoom_y = pic.zoom_y * scale
    pic.x = relOX + (pic.x - relOX) * scale
    pic.y = relOY + (pic.y - relOY) * scale  
  end
end
#==============================================================================#
...
class SMObject
  ...
  def set(param,newVal=nil,key=""); end
  def setViewport(viewport); @spriteViewport = viewport; end
 ...
 #GETTERS
  ...
  def objectsList; return @objectsList; end
  def objectsList(key)
    ret =nil
    if @objectsList[key]
      ret = @objectsList[key]
...
class SMSprite < SMObject
  def initialize(x=0,y=0,path="",viewport=nil)
    super(viewport)
 ...
 #GETTERS:
  ...
  #SETTERS:
  def addObj(key,x,y,path="",viewport=@spriteViewport)
    @objectsList[key]=SMSprite.new(@sprite.x+x,@sprite.y+y,path,viewport)
  end
  
  #def addObj(key,component); @objectsList[key] = component; end
  
  def disposed?; return @sprite.disposed?; end
  def dispose; @sprite.dispose; end
  
  def self.addObj(x,y,path,rx=0,ry=0,viewport=nil)
    return SMSprite.new(rx+x,ry+y,path,viewport)
  end
  ...
  def set(param,newVal=nil,key="")
    ret = false
    if key == ""
      case param
        when "bitmap"
          @sprite.bitmap=Bitmap.new(newVal)
          ret = true unless key != ""
        when "set_src_rect"
          @sprite.src_rect.set(newVal[0],newVal[1],newVal[2],newVal[3])
          ret = true unless key != ""
        when "setFont"
          pbSetSystemFont(@sprite.bitmap)
          ret = true
        when "drawText"
          pbDrawTextPositions(@sprite.bitmap,newVal)
          ret = true unless key != ""
        when "drawFText";
          drawFormattedTextEx(@sprite.bitmap,newVal[0],newVal[1],newVal[2],newVal[3],newVal[4],newVal[5])
          ret = true unless key != ""
        when "x";           @sprite.x=newVal
        when "y";           @sprite.y=newVal
        when "z";           @sprite.z=newVal
        when "clear";       @sprite.bitmap.clear
        when "ox";          @sprite.ox=newVal
        when "oy";          @sprite.oy=newVal
        when "zoom_x";      @sprite.zoom_x=newVal
        when "zoom_y";      @sprite.zoom_y=newVal
        when "visible";     @sprite.visible=newVal
        when "opacity";     @sprite.opacity=newVal
        when "wave_amp";    @sprite.wave_amp=newVal
        when "wave_length"; @sprite.wave_length=newVal
        when "wave_phase";  @sprite.wave_phase=newVal
        when "wave_speed";  @sprite.wave_speed=newVal
      end
      
      for i in @objectsList.keys
        break if ret
        ret = @objectsList[i].set(param,newVal)
      end
    else
      if @objectsList[key]
        ret = @objectsList[key].set(param,newVal)
      else
        for i in @objectsList.keys
          break if ret
          ret = @objectsList[i].set(param,newVal,key)
        end        
      end
    end
  ...
  #meant for zoom_x, zoom_y, and angle, but applicable for any other
  def setAnchorProp(property, newVal,ox=@sprite.ox,oy=@sprite.oy)
    for key in @objectsList.keys
      auxOX = @objectsList[key].ox
      auxOY = @objectsList[key].oy
      @objectsList[key].ox=ox-@objectsList[key].x
      @objectsList[key].oy=oy-@objectsList[key].y
      
      set(param,newVal,key)
      @objectsList[key].setAnchorProp(property, newVal,@objectsList[key].ox,@objectsList[key].oy)
      
      @objectsList[key].ox = auxOX
      @objectsList[key].oy = auxOY
    end
  end
  ...
  def fadeInOutSprite(sprite,frames,mode,delay=2)
    ...
    if sprite == nil
      ...
      begin
        ...
        for i in 0...frames
          obj.set("opacity",obj.opacity+mode*255/frames)
          pbWait(delay)
        end
        ok = true
      rescue
        pbThrow(_INTL("value at parameter frames of fadeInOutSprite set to 0. Don't do that."),false)
  ...
  def leftClick?(obj=nil)
    ok = nil
    if obj == nil; ok = $mouse.leftClick?(@sprite)
    elsif @objectsList[obj]; ok = $mouse.leftClick?(@objectsList[obj].sprite)
    else
      for i in @objectsList.keys
        ok = @objectsList[i].leftClick?(obj)
        break if ok != nil
      end
    end
    return ok
  ...
  def rightClick?(obj=nil)
    ok = nil
    if obj == nil; ok = $mouse.rightClick?(@sprite)
    elsif @objectsList[obj]; ok = $mouse.rightClick?(@objectsList[obj].sprite)
    else
      for i in @objectsList.keys
        ok = @objectsList[i].rightClick?(obj)
        break if ok != nil
      end
    end
    return ok
...
# Just don't touch ============================================================#
 ...
 $introName = ""
 $topScreen = SMScreen.new("","",0,0,Viewport.new(TOP_SCREEN_X,TOP_SCREEN_Y,SM_SCREEN_SYSTEM_WIDTH,SM_SCREEN_SYSTEM_HEIGHT))
 $btmScreen = SMScreen.new("","",0,0,Viewport.new(BTM_SCREEN_X,BTM_SCREEN_Y,SM_SCREEN_SYSTEM_WIDTH,SM_SCREEN_SYSTEM_HEIGHT))
#==============================================================================#
